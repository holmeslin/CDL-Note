...
機器學習是AI的一個子集，使機器可以從數據中學習，而不需要明確編程。
...
Google將生成式AI應用於Google Workspace等產品，幫助用戶輕鬆自動化不同類型的任務，例如生成長文檔的摘要。
...
Google還向開發者提供開發工具包，如生成式AI API，幫助他們創建定制的產品和服務。
...
生成式AI可以應用於多種應用程序，如對話機器人、內容生成、文檔合成和產品發現。
...
Maya領導一家國際航空公司的業務戰略和運營團隊，為了確立客戶購買模式的趨勢，她正在查看歷史年度報告。
...
有了所有這些不同的數據點，Maya可以預測單個航班的質量及其客戶投訴。
...
對Maya來說，真正的價值在於能夠全年對所有航班進行預測性洞察。
...
有了合適的數據，Maya可以使用機器學習來揭示這些類型的預測洞察，從而使航空公司及其客戶受益。
...
Ananda Development決定創建一個移動應用程序，以使這個過程更加高效。
...
此外，Ananda Development還希望通過使用遠程駕駛無人機，建立使用機器學習完成公寓檢查的途徑。
...
他們決定使用Cloud Vision API來自動化這個過程，以捕捉缺陷圖像並自動分類每個缺陷的信息。
...
實施三個月內，Ananda Development節省了約130小時的檢查時間和超過10萬美元的人力成本。
...
為了改進和擴展這個過程，Ocado利用機器學習處理自然語言的能力來識別客戶的情緒和每封郵件的主題，從而能夠立即將其路由到相關部門。
...
無效數據的一個例子是08-12-2019，當標準格式定義為年、月和日時。
...
例如，在一個客戶信息數據集中，同一個客戶在一個地方顯示為John Smith，在另一個地方顯示為J.Smith。
...
### 3.1.5 The importance of responsible and explainable AI
負責任和可解釋的人工智能的重要性
...
為此，Google制定了指導Google AI應用的原則，分享我們與Google外部社區的最佳實踐，並計劃合理化我們的努力。
...
除了這些原則外，Google不會在以下應用領域設計或部署人工智能：
...
儘管這些是Google自己的人工智能指導原則，我們敦促其他組織制定自己的原則，鼓勵負責任的人工智能開發。
...
儘管BigQuery最初僅作為數據倉庫，但隨著時間的推移，它已經發展為提供支持數據到人工智能生命周期的額外功能。
...
BigQuery ML使數據分析師（主要是數據倉庫用戶）能夠使用現有的商業智能工具和電子表格來構建和運行模型，從而實現機器學習的民主化。
...
它還提高了生產速度，因為在BigQuery中訓練模型不需要為基於Python的機器學習框架移動和格式化大量數據。

BigQuery ML also integrates with Vertex AI, Google Cloud's end-to-end AI and ML platform.
BigQuery ML還與Google Cloud的端到端人工智能和機器學習平台Vertex AI集成。
...
這些API在組織沒有專業數據科學家但有業務分析師和開發人員的情況下是理想的選擇。
...
Google Cloud的預訓練API通過提供用於分析圖像、視頻和文本等常見任務的機器學習模型，幫助開發人員快速構建智能應用程序。
...
無論機器學習經驗水平如何，API都可以部署在虛擬私有雲、本地或Google的公共雲中。
...
這提供了強大的預訓練機器學習模型，使用Google數據自動檢測圖像中的人臉、物體、文本甚至情感。
...
開發人員可以使用Vision API為圖像分配標籤，並將其快速分類到數百萬個預定義類別中。

The natural language API is another out-of-the-box, pre-trained API.
自然語言API是另一個開箱即用的預訓練API。
...
自然語言API發現文本中的語法、實體和情感，並將文本分類到預定義的類別集中。
...
正如您所期望的那樣，Google擁有大量的圖像、文本和機器學習研究人員來訓練其預訓練模型。
...
AutoML和Vertex AI允許您使用圖形用戶界面（通常稱為GUI）從端到端構建和訓練機器學習模型，而無需編寫一行代碼。
...
這意味著在您的數據被輸入Vertex AI後，AutoML會通過比較不同的模型和調整參數來為您選擇最佳機器學習模型。
...
Vision API可以告訴您Google數據庫中常見圖像之間的區別，例如車輪和車門之間的區別。
...
在這種情況下，開發人員可以使用AutoML視覺實例並用您的專業數據訓練它。
...
這自動化了機器學習模型的訓練，這意味著您可以上傳一批圖像，並通過易於使用的AutoML圖形界面訓練一個圖像分類模型。
...
但如果您的文本示例不能很好地適應自然語言API的基於情感或垂直主題的分類方案，並且您想使用自己的專業數據，則需要使用AutoML自然語言。
...
Vertex AI也是創建自定義端到端機器學習模型的基本平台。
...
Vertex AI提供了一套產品來幫助機器學習工作流程的每個階段，從收集數據到特徵工程、構建模型，最後是部署和監控這些模型。
...
Vertex AI包含幫助程序員進行虛擬機成像、數據標註、訓練和預測的工具。
...
TensorFlow擁有靈活的工具、庫和社區資源生態系統，使研究人員能夠在機器學習中創新，開發人員可以構建和部署機器學習驅動的應用程序。

First developed for Google's internal use, TensorFlow is now open source so that everyone can benefit.
TensorFlow最初是為Google內部使用而開發的，現在已經開源，讓每個人都能受益。
...
這意味著，使用最先進的GPU等待結果26小時，而使用完整的雲TPU pod僅需等待7.9分鐘即可獲得相同的結果。
...
雲TPU已經集成到Google產品中，這種最先進的硬件和超級計算技術可在Google Cloud產品和服務中使用。
...
除了可定制的選項外，Google Cloud還創建了一組完整的AI解決方案，旨在解決特定的業務需求。
...
聯絡中心AI提供與客戶對話和協助人員的模型，提高運營效率，個性化客戶服務，改變您的聯絡中心。
...
文檔AI通過從發票、收據、表格、信件和報告等非結構化文檔中提取和分類信息來解鎖洞察。
...
零售業的Discovery AI使用機器學習來選擇零售商電子商務網站上商品的最佳排序，當購物者選擇像冬季夾克或廚具這樣的類別時。
...
隨著時間的推移，AI使用歷史數據學習每個頁面的理想產品排序，優化展示產品的方式和內容，提高準確性、相關性和銷售可能性。
...
雲人才解決方案使用AI進行求職和人才獲取，能更快地將候選人與理想的工作匹配，並允許雇主吸引和轉化更高質量的候選人。

These are just some of the fully built AI solutions offered by Google Cloud.
這些只是Google Cloud提供的一些完整的AI解決方案。


### 3.2.7 Considerations when selecting Google Cloud AI/ML solutions
選擇Google Cloud AI/ML解決方案時的考慮因素
...
Google Cloud提供了一系列AI和ML解決方案和產品，但在選擇使用哪種解決方案時，需要考慮幾個決策和權衡。
...
AI項目通常需要3到36個月的時間來計劃和實施，具體取決於用例的範圍和複雜性。
...
自定義訓練通常需要最長時間，因為它從頭開始構建ML模型，不像AutoML和BigQuery ML。
...
Google Cloud為希望在日常業務運營中快速使用ML模型的組織提供了一系列現成的解決方案。
...
或者，Vertex AI是Google Cloud構建、部署和管理AI解決方案的統一平台，可以讓ML工程師和數據科學家完全控制ML工作流程。
...
Vertex AI自定義訓練允許您在Vertex Workbench上使用代碼訓練和提供自定義模型，從而生成高度定制的ML模型。

Another consideration is the expertise required when embarking on an AI or ML project.
另一個考慮因素是啟動AI或ML項目所需的專業知識。
...
將AI融入業務流程需要數據工程師、數據科學家和機器學習工程師等角色。
...
Google Cloud的AI和ML產品從可以由數據分析師和商業智能團隊使用的產品，到更適合ML工程師和數據科學家的產品不等。

The final consideration is the effort required to build an AI solution.
最後一個考慮因素是構建AI解決方案所需的努力。
...
Google Cloud可以幫助提供適用於不同規模項目的解決方案。
...
然而，任何AI項目通常都需要大量時間、努力和專業知識，才能對業務運營產生有價值的影響。
...
這讓我們來到了Google Cloud人工智能創新課程的結尾，讓我們快速回顧一下。
